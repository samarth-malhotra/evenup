...
> {
  ...
  >(
    ...
    // options (everything that used to be inside the useMutation config except mutationFn)
    {
      onMutate: async (payload) => {
        const { userId, nickname, theme } = payload;
        const key = QUERY_KEYS.user.profile(userId);
      ...
      onError: (err, variables, context) => {
        const key = QUERY_KEYS.user.profile(variables.userId);
