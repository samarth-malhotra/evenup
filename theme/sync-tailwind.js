...
(function main() {
  ...
  if (!tokensObj) {
    console.error(
      'Could not find tokens.ts or tokens.js in lib/theme. Please ensure one of these exists.\n' +
  ...
  const exportDest = path.join(__dirname, '..', 'theme', 'tokens-export.json');
  ...
  const colorsDest = path.join(__dirname, '..', 'theme', 'tailwind-colors.json');
